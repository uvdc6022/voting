...
**Eight pages, one shared shell.** `index.html` is the login page (loads `js/auth.js`). Every other page — `dashboard.html`, `cast-vote.html`, `users.html`, `voter-records.html`, `admins.html`, `course-monitoring.html`, `final-results.html` — shares the same layout (`<aside id="sidebar-root">` + `<main>` content root) and each loads its own same-named module (`js/dashboard.js`, `js/cast-vote.js`, etc.) as the sole page-specific script.
...
- `sidebar.js`'s `NAV_ITEMS` (`roles: [...]` per item) and `initSidebar()` — which nav links a role sees, and a hard redirect if a role-1 voter tries to navigate anywhere except `cast-vote.html`. `admins.html`, `voter-records.html`, and `course-monitoring.html` additionally self-check for role 3 in their own module (`admins.js`, `voter-records.js`, `course-monitoring.js`) and bounce non-super-admins to the dashboard.
...
**Course monitoring (`js/course-monitoring.js`, role 3 only):** Live-subscribes to `users/` and `votes/` (same `onValue` pattern as `dashboard.js`, not a one-shot `get()`) and groups non-super-admin users by `course` + `level` (e.g. "BEED 1") into one card per group showing Total, Voted, Not Voted, and percentage voted — voted status is `votes/{idNumber}` existence, same source of truth as everywhere else. Reuses the `dash-main`/`dash-results-header` shell (not `users-main`) so the card grid runs the full content width instead of the centered/capped-width treatment those pages use.
